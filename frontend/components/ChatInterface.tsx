...
export default function ChatInterface() {
  ...
  async function send() {
    ...
    const userMsg: Message = { id: ++counter.current, role: "user", content: q };
      ...
      <div className="shrink-0 px-6 py-4 border-b border-border flex items-center gap-3">
        <div className="flex-1 min-w-0">
          {doc ? (
            ...
            <>
              <p className="text-sm font-medium">Chat global</p>
          ...
          <div className="flex flex-col items-center justify-center h-full text-center gap-3 -mt-8">
            ...
            <div>
              ...
              <p className="text-xs text-muted-foreground mt-1">
                {doc ? `Sur "${doc.filename}"` : "Sur tous vos documents"}
              </p>
            </div>
          </div>
        )}
            ...
            {msg.role === "assistant" && (
              <div className="w-7 h-7 rounded-lg bg-primary/20 flex items-center justify-center shrink-0 mt-0.5">
                <svg width="13" height="13" viewBox="0 0 13 13" fill="none">
                  <path d="M2 2h9v7H7L4.5 12V9H2V2Z" stroke="currentColor" strokeWidth="1.2" className="text-primary" fill="none"/>
                </svg>
              </div>
            )}
